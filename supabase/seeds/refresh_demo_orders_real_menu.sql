...
begin
  ...
  where order_item_id in (
    ...
    where o.notes in ('[demo-prod-real]', '[demo-delivery-real]')
  );
  ...
  where order_id in (
    select id
    from public.orders
    where notes in ('[demo-prod-real]', '[demo-delivery-real]')
  );
  ...
  values (
    ...
    'R-201', '[demo-prod-real]', now() - interval '6 minutes'
  )
  ...
  values (
    order_id, 'coffee-1', 'Colibri Roxo com Iogurte', 'M', 'Iogurte natural', 27.90, 1, 'Sem banana'
  )
  ...
  values (
    ...
    'R-202', now() - interval '11 minutes', now() - interval '5 minutes',
    '[demo-prod-real]', now() - interval '13 minutes'
  )
  returning id into order_id;
  ...
  values (
    order_id, 'caribe-1', 'Limonada de Coco', 'G', 'Coco', 19.90, 1, 'Pouco gelo'
  )
  returning id into item_id;
  ...
  values (
    ...
    'R-203', now() - interval '18 minutes', now() - interval '11 minutes',
    now() - interval '2 minutes', '[demo-prod-real]', now() - interval '20 minutes'
  )
  returning id into order_id;
  ...
  values (
    order_id, 'a01', 'Açaí Bowl', 'G', 'Açaí tradicional', 33.90, 1, 'Pouco mel'
  )
  returning id into item_id;
  ...
  values (
    ...
    now() - interval '2 minutes', null, '[demo-delivery-real]', now() - interval '22 minutes'
  )
  returning id into order_id;
  ...
  values (
    order_id, 'coffee-1', 'Tropical Coffee', 'G', null, 35.90, 1, 'Sem gelo'
  )
  returning id into item_id;
