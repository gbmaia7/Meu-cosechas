...
export default function AssinaturaAtiva() {
  const navigate = useNavigate();
  const location = useLocation();
  const { totalItems, isAuthenticated } = useCart();
  const [isSubsActive, setIsSubsActive] = useState(true);
      ...
      <main className="max-w-md mx-auto pt-20 px-5 space-y-8">
        ...
        <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#bd002a] to-[#e8173a] p-8 text-white shadow-xl">
          ...
          <div className="space-y-1">
            ...
            <h2 className="text-4xl font-display font-extrabold tracking-tight">Plano Trio</h2>
          </div>

          <div className="mt-6 flex flex-col gap-1.5">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-white/90" fill="currentColor" stroke="none" />
              <span className="text-sm font-medium">12 compras/mês</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-white/90" fill="currentColor" stroke="none" />
              <span className="text-sm font-medium">15% de desconto</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-white/90" fill="currentColor" stroke="none" />
          ...
          <div className="mt-8 flex items-baseline gap-1">
            <span className="text-2xl font-black">R$165,00</span>
        ...
        <section className="bg-white rounded-2xl p-6 shadow-sm border border-[#f0eded]">
          <div className="flex justify-between items-end mb-4">
            ...
            <span className="text-[#bd002a] font-display font-bold text-lg">
              4 <span className="text-[#5d3f3e] font-medium text-sm">/ 12</span>
            </span>
          </div>
          ...
          <p className="mt-3 text-right text-sm font-medium text-[#5d3f3e]">8 unidades restantes</p>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#f0eded] shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-sm font-medium text-[#5d3f3e]">Próxima cobrança: <span className="font-bold text-[#1c1b1b]">01/05 · R$165,00</span></p>
            </div>
        ...
        {!isCanceled && (
          <section className="space-y-6 pt-4 pb-12">
            <div className="space-y-3 text-center">
              <button 
                onClick={() => navigate('/assinatura/trocar', { state: { currentPlan: 'Trio' } })}
            ...
            <button 
              onClick={() => navigate('/assinatura/cancelar', { state: { currentPlan: 'Trio' } })}
